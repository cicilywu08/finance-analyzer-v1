...
function detectYearMonthRegex(
  ...
  // Use more text so Amex "Closing Date" (often after summary box) is found
  const early = t.slice(0, 4500);

  // 1) Closing Date (allow no space after "Date" for Amex: "Closing Date02/13/23")
  ...
  // 3b) BOA-style "for August 19, 2025 to September 17, 2025" → statement = end date month (September 2025)
  const boaRange = /for\s+\w+\s+\d{1,2},?\s+\d{4}\s+to\s+(\w+)\s+(\d{1,2}),?\s+(\d{4})/i.exec(early);
  if (boaRange) {
    const yearVal = parseInt(boaRange[3], 10);
    const monthIdx = MONTH_NAMES.indexOf((boaRange[1] ?? "").toLowerCase());
    if (monthIdx >= 0) {
      const m = parseMonthYear(monthIdx + 1, yearVal);
      if (m) return { year: m.year, month: m.month, detectionSource: "period_range_boa", matchedText: boaRange[0] };
    }
  }
...
export { parseBoaTransactions } from "./parser/boa/parseTransactions";
export type { ParseBoaTransactionsResult } from "./parser/boa/parseTransactions";
export { parseAmexTransactions } from "./parser/amex/parseTransactions";
export type { ParseAmexTransactionsResult } from "./parser/amex/parseTransactions";
