...
function getPool(): Pool {
  ...
  const isDO =
    ...
    url.includes("db-postgresql");

  // For DO: use sslmode=no-verify in URL and rejectUnauthorized: false in pg so
  // TLS is used but self-signed cert is accepted (DO managed DB uses self-signed CA).
  let connectionString = url;
  if (isDO) {
    connectionString = url.replace(/sslmode=require/gi, "sslmode=no-verify");
    if (!connectionString.includes("sslmode=")) {
      connectionString += (url.includes("?") ? "&" : "?") + "sslmode=no-verify";
    }
  }

  const sslConfig = isDO ? { rejectUnauthorized: false } : undefined;
  const redacted = connectionString.replace(/:\/\/([^:]+):([^@]+)@/, "://***:***@");
  console.log("[DB] getPool() once: isDO =", isDO, "connectionString =", redacted, "ssl =", sslConfig ? "rejectUnauthorized:false" : "undefined");
...
/** Delete all data for a statement month: transactions and month_overrides. */
