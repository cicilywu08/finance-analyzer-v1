...
/** Success payload (returned only when we have full summary data). */
export interface SummarySuccessPayload {
...
export async function GET(): Promise<Response> {
  ...
  if (!process.env.DATABASE_URL) {
    return Response.json(
      { ok: false, error: "Database not configured." } satisfies SummaryResponse,
      { status: 200, headers }
    );
  }
  try {
    ...
    if (months.length < 2) {
      return Response.json(
        { ok: false, error: "Summary requires at least 2 months of data." } satisfies SummaryResponse,
        { status: 200, headers }
      );
    }
    ...
    return Response.json(
      { ok: false, error: err instanceof Error ? err.message : "Failed to load summary." } satisfies SummaryResponse,
      { status: 200, headers }
    );
  }
}
