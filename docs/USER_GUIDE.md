# Finance Analyzer — User Guide

This guide is for people who want to **run Finance Analyzer on their own computer**. Your statements and spending data stay on **your** machine (and in **your** database). You bring your own OpenAI API key if you want AI-powered categories and insights.
...
After installing, you still follow this guide’s steps to **create an empty database** (one command like `createdb finance_analyzer`) and then put the matching `DATABASE_URL` in `.env.local`.
...
- Pay for a short session with a local tech-support service—bring this guide.
...
Still use `http://localhost:3000` (or the port shown in the terminal) unless you change `PORT`.
...
## Privacy and security (short checklist)
...
Problems that depend on your OS, Postgres install, or network are easiest to debug with the exact error message from the terminal or browser. Check the project **README** for maintainer-oriented setup and deployment notes.

---

*This guide describes self-hosted use. Features and supported banks may change; refer to the repository for the latest behavior.*
